...
try {
  ...
  const proposals = loadJson("demo/generated/batch-proposals.json");
  const receipt = loadJson("demo/generated/default-policy-receipt.json");
  const evidenceSets = loadJson("demo/generated/simulation-results.json");

  // Compute hashes using same canonical rules as the engine
  const receiptHash = sha256Hex(canonicalJson(receipt));
  const policyHash = sha256Hex(canonicalJson({
    policy_id: "marketlens-default-v1",
    max_actions: 5,
    execution_mode: "ALLOW_ELIGIBLE_ONLY"
  }));
  const allowlistHash = sha256Hex(canonicalJson({
    eligible_proposal_ids: receipt.eligible_proposal_ids || [],
    blocked_proposal_ids: receipt.blocked_proposal_ids || [],
    unsigned: true,
    not_broadcast: true
  }));
  ...
  const output = {
    ...
    source_artifact_paths: [
      "demo/generated/batch-proposals.json",
      "demo/generated/default-policy-receipt.json",
      "demo/generated/simulation-results.json"
    ],
