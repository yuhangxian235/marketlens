...
export async function POST(request: Request) {
  ...
  if (input.proposals.length < 2 || input.proposals.length > 5) {
    return NextResponse.json({ error: "proposals must contain 2-5 items" }, { status: 400 });
  }
  ...
  for (let i = 0; i < input.proposals.length; i++) {
    ...
    if (seenIds.has(id)) {
      return NextResponse.json({ error: `duplicate proposal_id: "${id}"` }, { status: 400 });
    }
    seenIds.add(id);
  }
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Live verification failed", detail: message },
      { status: 502 }
    );
  }
}
